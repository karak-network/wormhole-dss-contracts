...
contract NativeVault is ERC4626, IBeacon, Pauser, INativeVault, OwnableRoles, ReentrancyGuard {
    ...
    function initialize(
        ...
        if (_depositToken != Constants.NATIVE_ASSET_ADDR) revert DepositTokenNotAccepted();

        (address manager, address nodeImplementation) = abi.decode(_extraData, (address, address));

        if (manager == address(0) || nodeImplementation == address(0)) revert ZeroAddress();
        ...
        emit NativeVaultInitialized(_owner, manager, _operator);
    }
        ...
        whenFunctionNotPaused(Constants.PAUSE_NATIVEVAULT_VALIDATE_SNAPSHOT)
    {
        ...
        for (uint256 i = 0; i < balanceProofs.length; i++) {
            ...
            if (validatorDetails.status != NativeVaultLib.ValidatorStatus.ACTIVE) continue;
        ...
        whenFunctionNotPaused(Constants.PAUSE_NATIVEVAULT_VALIDATE_WITHDRAW_CREDS)
    {
        ...
        for (uint256 i = 0; i < validatorFieldsProofs.length; i++) {
            totalRestakedWei += self.validateWithdrawalCredentials(
                nodeOwner, beaconStateRootProof.beaconStateRoot, validatorFieldsProofs[i]
            );
        }
    ...
    /// @param slashingHandler: address of the slash store, ignored in this case
    ...
    /// @notice fetches paused map for the vault
    function pausedMap() public view override(IKarakBaseVault, Pauser) returns (uint256) {
        return super.pausedMap();
    }
    /* ======================================== */
