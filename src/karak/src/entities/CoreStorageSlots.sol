...
uint256 constant VAULT_QUEUED_SLASHING_MAPPING_OFFSET = 7;
...
library CoreStorageSlots {
    ...
    function vaultQueuedSlashingMappingSlot(address operator) internal pure returns (bytes32) {
        return bytes32(uint256(operatorStateSlot(operator)) + VAULT_QUEUED_SLASHING_MAPPING_OFFSET);
    }
    ...
    function vaultQueuedSlashingSlot(address operator, address vault) public pure returns (bytes32) {
        return keccak256(abi.encode(vault, vaultQueuedSlashingMappingSlot(operator)));
    }
}
