...
contract OperatorDSS is OperatorHelper {
    ...
    function test_fail_register_operator_to_unregistered_dss(address dssTest) public {
        vm.assume(!core.isDSSRegistered(IDSS(dssTest)));
    ...
    function test_request_stake_update_request(uint256 time) public {
        ...
        assertEq(core.fetchVaultsStakedInDSS(operator, dss).length, 2);
        ...
        assertEq(core.fetchVaultsStakedInDSS(operator, dss).length, 1);
    ...
    function test_fail_stake_update_post_operator_unregistration(uint256 time) public {
        deposit_into_vaults();
    ...
    function test_fail_stake_vault_already_staked_to_dss() public {
        stake_vaults_to_dss();
        Operator.StakeUpdateRequest memory stakeUpdate =
            Operator.StakeUpdateRequest({vault: address(vaults[0]), dss: dss, toStake: true});

        vm.expectRevert(VaultAlreadyStakedInDSS.selector);
        vm.prank(operator);
        core.requestUpdateVaultStakeInDSS(stakeUpdate);
    }

    function test_fail_unstake_vault_not_staked_to_dss() public {
        deposit_into_vaults();
        Operator.StakeUpdateRequest memory stakeUpdate =
            Operator.StakeUpdateRequest({vault: address(vaults[0]), dss: dss, toStake: false});

        vm.expectRevert(VaultNotStakedInDSS.selector);
        vm.prank(operator);
        core.requestUpdateVaultStakeInDSS(stakeUpdate);
    }
