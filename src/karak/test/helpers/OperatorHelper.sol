...
contract OperatorHelper is Test {
    ...
    function withdraw_all_shares(address withdrawer, Vault vault) public {
        withdraw_from_vault(withdrawer, vault, vault.balanceOf(withdrawer));
    }

    function withdraw_from_vault(address withdrawer, Vault vault, uint256 amount) public {
        vm.startPrank(withdrawer);
        vault.approve(address(vault), amount);
        bytes32 withdrawalKey = vault.startRedeem(amount, address(this));
